namespace Sitecore.FakeDb.Tests
{
  ...
  public class DbTest
  {
    ...
    [Fact]
    public void ShouldNotGenerateTemplateIfSetExplicitly()
    {
      // arrange
      using (var db = new Db
                        {
                          new DbTemplate("template", this.templateId),
                          new DbItem("explicit_template", ID.NewID, this.templateId),
                          new DbItem("autogenerated_template")
                        })
      {
        // act
        var outside = db.GetItem("/sitecore/content/autogenerated_template");

        // assert
        outside.TemplateID.Should().NotBe(this.templateId);
      }
    }
